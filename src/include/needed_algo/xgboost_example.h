...
void testXgboost() {
    ...
    (XGDMatrixCreateFromMat((float *) train, 4, 2, -1, &dtrain));
    (XGDMatrixSetFloatInfo(dtrain, "label", label, 4)); // set label parameter
    // Create booster
    ...
    (XGBoosterPredict(booster, dtest, 4, 2, 0, &out_len, &out_result));
    ...
    (XGBoosterPredict(booster, dtest, 4, 2, 1, &out_len, &out_result));
